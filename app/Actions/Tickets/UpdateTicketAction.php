...
'assigned_user_id' => $data['assigned_user_id'] !== '' ?
        $data['assigned_user_id'] : null, // lege string terug omzetten naar null
]);
$ticket->refresh(); // herlaad ticket zodat enums en relaties up-todate zijn

if ($oldSubject !== $ticket->subject) {
    $this->logTicketActivityAction->execute(
        $ticket,
        "Onderwerp gewijzigd van '{$oldSubject}' naar
'{$ticket->subject}'." // log onderwerpwijziging
    );
}
...
if ($oldStatus !== $ticket->status) {
    $this->logTicketActivityAction->execute(
        $ticket,
        "Status gewijzigd van {$oldStatus->value} naar
{$ticket->status->value}." // log statuswijziging
    );
}
if ($oldPriority !== $ticket->priority) {
    $this->logTicketActivityAction->execute(
        $ticket,
        "Prioriteit gewijzigd van {$oldPriority->value} naar
{$ticket->priority->value}." // log prioriteitswijziging
    );
}
if ($oldWorkflow !== $ticket->workflow_step) {
    $this->logTicketActivityAction->execute(
        $ticket,
        "Workflow gewijzigd van {$oldWorkflow->value} naar
{$ticket->workflow_step->value}." // log workflowwijziging
    );
}
...
{$ticket->assigneeName()}." // log assignmentwijziging
    );
}
