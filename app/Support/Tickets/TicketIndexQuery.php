...
$subQuery->where('subject', 'like', '%' .
    $filters['search'] . '%')
    ->orWhere('description', 'like', '%' .
        $filters['search'] . '%'); // zoek in onderwerp of beschrijving
});
            })
            ->when(($filters['status'] ?? '') !== '', function ($query) use
            ($filters) {
                $query->where('status', $filters['status']); // filter op status
})
            ->when(($filters['priority'] ?? '') !== '', function ($query) use
            ($filters) {
                $query->where('priority', $filters['priority']); // filter op prioriteit
})
            ->when(($filters['workflow'] ?? '') !== '', function ($query) use
            ($filters) {
                $query->where('workflow_step', $filters['workflow']); // filter op workflow
})
            ->when(($filters['assigned_user_id'] ?? '') !== '', function
            ($query) use ($filters) {
                $query->where('assigned_user_id',
                    $filters['assigned_user_id']); // filter op assignee
            })
            ->orderBy(
