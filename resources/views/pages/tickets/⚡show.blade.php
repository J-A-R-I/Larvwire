...
class extends Component
{
    ...
    public function mount(Ticket $ticket): void
    {
        ...
        $this->subject = $ticket->subject; // laad onderwerp in componentstate
        ...
        $this->priority = $ticket->priority; // laad prioriteit in componentstate
        $this->workflow_step = $ticket->workflow_step; // //NIEUW: laad workflow in component-state
        $this->assigned_user_id = $ticket->assigned_user_id ? (string)
        $ticket->assigned_user_id : ''; // //NIEUW: laad assignee of lege string
}
    #[Computed]
    ...
    public function save(): void
    {
        ...
        $this->ticket->update([
            ...
            'assigned_user_id' => $validated['assigned_user_id'] !== '' ?
                $validated['assigned_user_id'] : null, // //NIEUW: zet lege string om naar
            null
        ]);
        $this->ticket->refresh(); // herlaad nieuwste ticketwaarden uit database
        if ($oldStatus !== $this->ticket->status) {
            $this->ticket->logActivity(
                "Status gewijzigd van {$oldStatus} naar {$this->ticket->status}." // //NIEUW: log statuswijziging
            );
        }
        if ($oldPriority !== $this->ticket->priority) {
            $this->ticket->logActivity(
                "Prioriteit gewijzigd van {$oldPriority} naar
                {$this->ticket->priority}." // //NIEUW: log prioriteitswijziging
            );
        }
        if ($oldWorkflow !== $this->ticket->workflow_step) {
            $this->ticket->logActivity(
                "Workflow gewijzigd van {$oldWorkflow} naar
{$this->ticket->workflow_step}." // //NIEUW: log workflowwijziging
            );
        }
        if ($oldAssigneeName !== $this->ticket->assigneeName()) {
            $this->ticket->logActivity(
                "Toegewezen behandelaar gewijzigd van {$oldAssigneeName} naar
{$this->ticket->assigneeName()}." // //NIEUW: log assignmentwijziging
            );
        }
...
<div class="min-h-screen bg-gray-100 py-10">
    <div class="mx-auto max-w-4xl px-4">
        <div class="mb-6 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            <div class="flex items-center gap-3">
                ...
                <button
                    ...
                    wire:target="delete"
                    class="inline-flex items-center rounded-lg bg-red-600 px4 py-2 text-sm font-semibold
        ...
        <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
            <form wire:submit="save" class="space-y-6">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div>
                        <label for="assigned_user_id" class="mb-2 block textsm font-medium text-gray-700">
                            Toegewezen aan
                        </label>
                        <select
                            id="assigned_user_id"
                ...
                <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-700">
                    <div class="grid gap-3 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            {{ $ticket->assigneeName() }} {{-- //NIEUW: toon
                            actuele toegewezen gebruiker --}}
                        </div>
                    </div>
                </div>
                <div class="flex items-center gap-4">
                    <button
                        ...
                        class="inline-flex items-center rounded-lg bg-blue-600 px-5 py-3 text-sm font-semibold
                         text-white shadow-sm transition hover:bgblue-700 disabled:cursor-not-allowed disabled:opacity-50">
                        Wijzigingen opslaan
                    </button>
        ...
        <div class="mt-6 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray200">
