...
'content.min' => 'De inhoud moet minstens 3 tekens
bevatten.', // foutmelding voor te korte inhoud
...
$this->dispatch('comment-created', commentId: $comment->id, ticketId:
    $this->ticket->id); // dispatch event voor andere componenten op de pagina
...
$this->dispatch('comment-deleted', commentId: $commentId, ticketId:
    $this->ticket->id); // dispatch event na delete
...
@if (session()->has('comments_success')) {{-- toon succesfeedback na save of delete --}}
    ...
    <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200"> {{-- kaart met formulier voor nieuwe reactie --}}
        ...
        <form wire:submit="save" class="space-y-4"> {{-- formulier dat save() uitvoert zonder reload --}}
            <div> {{-- blok voor type --}}
                <label for="type" class="mb-2 block text-sm font-medium textgray-700"> {{-- label voor type --}}
                    Type {{-- labeltekst --}}
                </label>
                <select
                    id="type"
...
@error('content') {{-- foutmelding voor inhoud --}}
            ...
            <div class="flex items-center gap-4"> {{-- acties onder het formulier --}}
                ...
                <span wire:loading wire:target="save" class="text-sm textgray-500"> {{-- loading feedback tijdens save --}}
    ...
    <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200"> {{-- kaart met historiek van reacties --}}
        ...
        <div class="space-y-4"> {{-- lijst met comments --}}
                ...
                <div class="rounded-xl border border-gray-200 bg-gray-50 p4"> {{-- kaart van één comment --}}
                    <div class="mb-3 flex flex-col gap-3 md:flex-row md:items-start md:justify-between"> {{-- bovenste rij per comment --}}
                        ...
                        <button
                            ...
                            class="inline-flex items-center rounded-lg bgred-600 px-3 py-2 text-xs font-semibold
    ...
    {{ $comment->content }} {{-- eigenlijke commenttekst --}}
            ...
            <div class="rounded-xl border border-dashed border-gray-300 px-4 py-8 text-center text-sm
             text-gray-500"> {{-- empty state --}}
                Er zijn nog geen reacties of interne notities toegevoegd
                voor dit ticket. {{-- lege melding --}}
            </div>
            @endforelse
        </div>
    </div>
